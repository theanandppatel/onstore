...
const Search = (fetchProducts, stockFlag) => {

  const queryProducts = fetchProducts.fetchProducts

  // const search = useSearchParmas()
  return (
    <>
      <Head>
        <title>Search page - Onstore</title>
      ...
      {Object.keys(queryProducts).length == 0 ? <p className='text-gray-600 text-center pt-40 pb-40'>No matching Item Found!. Try Different Search.</p> :
        <div className='pt-32 pb-36 md:ml-14 items-center lg:gap-4 lg:grid-cols-4 grid grid-cols-2' style={{ textAlign: "-webkit-center" }}>
          {Object.keys(queryProducts).map((item) => {
            return (
              <div className="max-w-xs shadow-lg rounded-xl p-6 relative group" key={queryProducts[item]._id}>
                    ...
                    <div className="">
                      <div className="">
                        ...
                        <div className="items-center w-full justify-between min-w-0 ">
                          <h2 className="text-base md:text-lg mr-auto cursor-pointer text-black hover:text-blue-700 overflow-hidden line-clamp-2"><Link href={`/products/fashion-products/${queryProducts[item].slug}`}>{queryProducts[item].title}</Link></h2>
                          {/* {queryProducts[item].availableQty > 0 ? <div className="flex items-center bg-green-600 text-white text-xs px-2 py-1 ml-3 rounded-lg">INSTOCK</div>
                                ...
                                return (
                                  <li key={k}>
                                    <span className={`block p-1 border-2 border-white hover:border-${k}-600 rounded-full transition ease-in duration-300`}>
                                      <a href={`/products/fashion-products/${queryProducts[item].slug}`} className={`block w-3 h-3rounded-full`} style={{backgroundColor: "black"}}></a>
                                    </span>
                                  </li>
                                )
                              })} */}

                              {queryProducts[item].color.includes('blue') && <li>
                                <span className="block p-1 border-2 border-white hover:border-blue-600 rounded-full transition ease-in duration-300">
                                  <a href={`/products/${queryProducts[item].category}/${queryProducts[item].slug}`} className="block w-3 h-3 bg-blue-600 rounded-full"></a>
                                </span>
                              </li>}
                              {queryProducts[item].color.includes('black') && <li>
                                <span className="block p-1 border-2 border-white hover:border-slate-900 rounded-full transition ease-in duration-300">
                                  <a href={`/products/${queryProducts[item].category}/${queryProducts[item].slug}`} className="block w-3 h-3 bg-slate-800 rounded-full"></a>
                                </span>
                              </li>}
                              {queryProducts[item].color.includes('white') && <li>
                                <span className="block p-1 border-2 border-white hover:border-gray-400 rounded-full transition ease-in duration-300">
                                  <a href={`/products/${queryProducts[item].category}/${queryProducts[item].slug}`} className="block w-3 h-3 bg-gray-300 rounded-full"></a>
                                </span>
                              </li>}
                              {queryProducts[item].color.includes('yellow') && <li>
                                <span className="block p-1 border-2 border-white hover:border-yellow-400 rounded-full transition ease-in duration-300">
                                  <a href={`/products/${queryProducts[item].category}/${queryProducts[item].slug}`} className="block w-3 h-3  bg-yellow-400 rounded-full"></a>
                                </span>
                              </li>}
                              {queryProducts[item].color.includes('red') && <li>
                                <span className="block p-1 border-2 border-white hover:border-red-500 rounded-full transition ease-in duration-300">
                                  <a href={`/products/${queryProducts[item].category}/${queryProducts[item].slug}`} className="block w-3 h-3  bg-red-500 rounded-full"></a>
                                </span>
                              </li>}
                              {queryProducts[item].color.includes('green') && <li>
                                <span className="block p-1 border-2 border-white hover:border-green-500 rounded-full transition ease-in duration-300">
                                  <a href={`/products/${queryProducts[item].category}/${queryProducts[item].slug}`} className="block w-3 h-3  bg-green-500 rounded-full"></a>
                                </span>
                              </li>}
                              {queryProducts[item].color.includes('purple') && <li>
                                <span className="block p-1 border-2 border-white hover:border-purple-500 rounded-full transition ease-in duration-300">
                                  <a href={`/products/${queryProducts[item].category}/${queryProducts[item].slug}`} className="block w-3 h-3  bg-purple-500 rounded-full"></a>
                                </span>
                              </li>}
                            </ul>
                          </div>
                        </div>
                        {queryProducts[item].size && <div className="flex-1 inline-flex items-center mb-0 md:mb-3">
                          <span className="text-secondary whitespace-nowrap mr-3">Size:</span>
                          <div className="cursor-pointer text-black">
                            {queryProducts[item].size.sort((a, b) => b.localeCompare(a)).map((k) => {
                              return (
                      ...
                      <div className="md:flex space-x-2 text-sm font-medium justify-between">
                        <div className="text-xl font-semibold mt-1">₹{queryProducts[item].price.toLocaleString('en-IN')}</div>
                        <Link href={`/products/${queryProducts[item].category}/${queryProducts[item].slug}`}>
...
export async function getServerSideProps(context) {
  ...
  const res = await fetch(`${process.env.NEXT_PUBLIC_HOST}/api/searchquery`, {
    method: 'POST',
    headers: {
  ...
  for (let item of result) {
    if (item.title in searchProducts) {

      if (!searchProducts[item.title].color.includes(item.color)) {
        searchProducts[item.title].color.push(item.color)
      }
      if (!searchProducts[item.title].size.includes(item.size) && item.availableQty > 0) {
        stockFlag = true;
        searchProducts[item.title].size.push(item.size)
      }

    } else {
      searchProducts[item.title] = JSON.parse(JSON.stringify(item))

      if (searchProducts[item.title].color != "") {
        searchProducts[item.title].color = [item.color]
      }


      if (searchProducts[item.title].size != "") {
        searchProducts[item.title].size = [item.size]
      }
  ...
  return {
    props: { fetchProducts: JSON.parse(JSON.stringify(searchProducts)), stockFlag: stockFlag },// will be passed to the page component as props
  }
}
