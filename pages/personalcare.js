...
const PersonalCare = ({ personalcare }) => {
  return (
    <>
      {Object.keys(personalcare).length == 0 ? <p className='text-gray-600 text-center pt-40 pb-40'>Sorry all the Beauty & Personal Care Items are currently out of stock. New stock coming soon. Stay Tuned!</p> :
        <div className='pt-32 pb-36 md:ml-14 items-center lg:gap-4 lg:grid-cols-4 grid grid-cols-2' style={{ textAlign: "-webkit-center" }}>
          {Object.keys(personalcare).map((item) => {
            return (
              <div className="max-w-xs shadow-lg rounded-xl p-6 relative group" key={personalcare[item]._id}>
                <div className="flex flex-col ">
                  <div>
                    {personalcare[item].availableQty > 0 ? <div className="absolute z-10 flex flex-col top-2 right-2 items-center bg-green-600 text-white text-xs px-2 py-1 ml-3 rounded-lg">INSTOCK</div>
                    ...
                    <div className="">
                      <div className="">
                        ...
                        <div className="items-center w-full justify-between min-w-0 ">
                          <h2 className="text-base md:text-lg mr-auto cursor-pointer text-black hover:text-blue-700 overflow-hidden line-clamp-2"><Link href={`/products/${personalcare[item].category}/${personalcare[item].slug}`}>{personalcare[item].title}</Link></h2>
                          {/* {personalcare[item].availableQty > 0 ? <div className="flex items-center bg-green-600 text-white text-xs px-2 py-1 ml-3 rounded-lg">INSTOCK</div>
                            : <div className="flex items-center bg-red-600 text-white text-xs px-2 py-1 ml-3 rounded-lg">OUTOFSTOCK</div>} */}
                        </div>
                      </div>
                      {/* <div className="lg:flex py-1 md:py-4  text-sm text-gray-600">
                        <div className="flex-1 md:inline-flex items-center  mb-3">
                          <div className="w-full flex-none text-sm flex items-center text-gray-600">
                            <ul className="flex flex-row justify-center items-center space-x-2">
                              {personalcare[item].color.includes('blue') && <li>
                                <span className="block p-1 border-2 border-white hover:border-blue-600 rounded-full transition ease-in duration-300">
                                  <a href={`/product/${personalcare[item].slug}`} className="block w-3 h-3 bg-blue-600 rounded-full"></a>
                                </span>
                              </li>}
                              {personalcare[item].color.includes('yellow') && <li>
                                <span className="block p-1 border-2 border-white hover:border-yellow-400 rounded-full transition ease-in duration-300">
                                  <a href={`/product/${personalcare[item].slug}`} className="block w-3 h-3  bg-yellow-400 rounded-full"></a>
                                </span>
                              </li>}
                              {personalcare[item].color.includes('red') && <li>
                                <span className="block p-1 border-2 border-white hover:border-red-500 rounded-full transition ease-in duration-300">
                                  <a href={`/product/${personalcare[item].slug}`} className="block w-3 h-3  bg-red-500 rounded-full"></a>
                                </span>
                              </li>}
                              {personalcare[item].color.includes('green') && <li>
                                <span className="block p-1 border-2 border-white hover:border-green-500 rounded-full transition ease-in duration-300">
                                  <a href={`/product/${personalcare[item].slug}`} className="block w-3 h-3  bg-green-500 rounded-full"></a>
                                </span>
                              </li>}
                            </ul>
                          </div>
                        ...
                        <div className="flex-1 inline-flex items-center mb-0 md:mb-3">
                          ...
                          <div className="cursor-pointer text-gray-400 ">
                            {personalcare[item].size.includes('S') && <span className="hover:text-purple-900 p-1 py-0">S</span>}
                            {personalcare[item].size.includes('M') && <span className="hover:text-purple-900 p-1 py-0">M</span>}
                            {personalcare[item].size.includes('L') && <span className="hover:text-purple-900 p-1 py-0">L</span>}{personalcare[item].size.includes('XL') && <span className="hover:text-purple-900 p-1 py-0">XL</span>}
                            {personalcare[item].size.includes('XXL') && <span className="hover:text-purple-900 p-1 py-0">XXL</span>}
                          </div>
                        </div>
                      </div> */}
                      <div className="md:flex space-x-2 text-sm font-medium justify-between mt-5">
                        <div className="text-xl font-semibold mt-1">₹{personalcare[item].price.toLocaleString('en-IN')}</div>
                        <Link href={`/products/${personalcare[item].category}/${personalcare[item].slug}`}>
...
export async function getServerSideProps(context) {
  ...
  let personalcare = await Product.find({category:'personal-and-beauty-care-products'})
  ...
  return {
    props: { personalcare: JSON.parse(JSON.stringify(personalcare)) }, // will be passed to the page component as props
  }
}
