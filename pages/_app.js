...
function MyApp({ Component, pageProps }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (localStorage.getItem("myuser")) {
        setUser({ value: JSON.parse(localStorage.getItem("myuser")).token,email: JSON.parse(localStorage.getItem("myuser")).email })
  ...
  const addToCart = (itemCode, image, qty, price, name, size, variant, category) => {
    let myCart = cart;
    if (itemCode in cart) {
      myCart[itemCode].qty = cart[itemCode].qty + qty;
    } else {
      myCart[itemCode] = { image, qty: 1, price, name, size, variant, category }
    }
    setCart(myCart);
    saveCart(myCart)
  }

  const removeFromCart = async (itemCode, image, qty, price, name, size, variant, category) => {
    let myCart = cart;
  ...
  const buyNow = (itemCode, image, qty, price, name, size, variant, category)=>{
    let newCart = {}
