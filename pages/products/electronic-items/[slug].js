...
const Electronicitems = ({ error, cart, clearCart, variants, addToCart, products, buyNow, productImgArr, prodHighlights, prodDetails}) => {
    ...
    const refreshVariant = (newSize, newColor) => {
        ...
        setColor(newColor)
        let url = `${process.env.NEXT_PUBLIC_HOST}/products/${products.category}/${variants[newColor]['slug']}`
        // window.location = url;
        router.push(url)
    }
    ...
    const handleBuyNow = (e) => {
        ...
        let slug = products.slug
        buyNow(slug, products.img[0], 1, products.price, products.title, "", color, products.category)
    }
                        ...
                        <ol role="list" className="mx-auto flex max-w-2xl items-center space-x-2 px-4 sm:px-6 lg:max-w-7xl lg:px-8">
                            <li>
                                <div className="flex items-center">
                                    <Link href={'/'}>
                                        <span className="mr-2 text-sm font-medium text-gray-900 cursor-pointer">Home</span>
                                    </Link>
                                    <svg
                                        width={16}
                                        height={20}
                            ...
                            <li>
                                <div className="flex items-center">
                                    <Link href={'/electronics'}>
                                        <span className="mr-2 text-sm font-medium text-gray-900 cursor-pointer">Electronic Items</span>
                                    </Link>
                                    <svg
                                        width={16}
                                        height={20}
                    ...
                    <div className="mx-auto max-w-2xl px-4 pt-3  sm:px-6 lg:grid lg:max-w-7xl lg:grid-cols-3 lg:grid-rows-[auto,auto,1fr] lg:gap-x-8 lg:px-8 lg:pb-24">
                        ...
                        <div className="mt-4 lg:row-span-3 lg:mt-0 px-5">
                            ...
                            <form className="mt-3">
                                ...
                                <div className="relative border-2 border-indigo-600 rounded-lg mt-32">
...
export async function getServerSideProps(context) {
    ...
    let featuredprod = await FeaturedProduct.findOne({ slug: context.query.slug })

    let recommendedprod = await RecommendedProduct.findOne({ slug: context.query.slug })

    if ((products == null || products.category != 'electronic-items') && (featuredprod == null || featuredprod.category != 'electronic-items') && (recommendedprod == null || recommendedprod.category != 'electronic-items')) {
        return {
    ...
    if (products != null) {
        variants = await Product.find({ title: products.title })
        imgVariants = await Product.find({ color: products.color, title: products.title })
        productImgArr = imgVariants[0].img
        prodHighlights = products.highlights
        prodDetails = products.details
    }
    else if (featuredprod != null) {
        variants = await FeaturedProduct.find({ title: featuredprod.title })
        imgVariants = await FeaturedProduct.find({ color: featuredprod.color, title: featuredprod.title })
        productImgArr = imgVariants[0].img
        prodHighlights = featuredprod.highlights
        prodDetails = featuredprod.details
        products = featuredprod
    } else {
        variants = await RecommendedProduct.find({ title: recommendedprod.title })
        imgVariants = await RecommendedProduct.find({ color: recommendedprod.color, title: recommendedprod.title })
        productImgArr = imgVariants[0].img
        prodHighlights = recommendedprod.highlights
        prodDetails = recommendedprod.details
    ...
    return {
        props: {prodDetails: JSON.parse(JSON.stringify(prodDetails)), prodHighlights: JSON.parse(JSON.stringify(prodHighlights)), productImgArr: JSON.parse(JSON.stringify(productImgArr)), products: JSON.parse(JSON.stringify(products)), variants: JSON.parse(JSON.stringify(colorSlug)) }, // will be passed to the page component as props
    }
}
